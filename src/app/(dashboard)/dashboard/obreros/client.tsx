...
export default function ObrerosRankingClient({ obreros }: { obreros: ObreroRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return obreros.filter((o) => {
      ...
      if (!term) return true;
      return [o.nombre, o.cedula, o.contratista, o.especialidad, o.telefono]
        .filter(Boolean)
        .some((v) => v!.toLowerCase().includes(term));
    });
  }, [obreros, search, especialidad, soloActivos]);
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar por nombre, cédula, contratista..."
            className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400"
          />
        </div>
        <select
          value={especialidad}
  ...
  return (
    <div className={`rounded-2xl border p-4 ${colors[pos - 1] ?? "bg-white"}`}>
      ...
      <div className="text-xs text-slate-500 truncate">
        {o.especialidad
          ? ESPECIALIDAD_LABEL[o.especialidad as keyof typeof ESPECIALIDAD_LABEL]
          : "Sin especialidad"}
      </div>
