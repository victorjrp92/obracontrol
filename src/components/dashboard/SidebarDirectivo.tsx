...
const navItems = [
  { key: "inicio", icon: LayoutDashboard, label: "Inicio", href: "/directivo" },
  { key: "proyectos", icon: FolderOpen, label: "Proyectos", href: "/directivo?tab=proyectos" },
];
...
export default function SidebarDirectivo({ userName = "Usuario" }: SidebarDirectivoProps) {
      ...
      <div className="flex items-center gap-2.5 px-4 h-16 border-b border-slate-800 flex-shrink-0">
        ...
        {!collapsed && (
          <div className="leading-tight">
            <div className="font-extrabold text-white text-base tracking-wide whitespace-nowrap">
              SEIRICON
            </div>
        ...
        <div className="px-4 py-3 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] text-slate-400 truncate">Directivo</div>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        <ul className="flex flex-col gap-1 px-2">
          {navItems.map((item) => {
            ...
            // For "Inicio", only active when exactly on /directivo with no tab
            // For "Proyectos" (query-based), active when tab=proyectos is in URL
            const active =
              item.key === "inicio"
                ? pathname === "/directivo"
                : pathname.startsWith("/directivo/proyecto");
