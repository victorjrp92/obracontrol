...
interface SidebarContratistaProps {
  userName?: string;
  userRole?: string;
}

export default function SidebarContratista({
  userName = "Contratista",
  userRole = "Contratista",
}: SidebarContratistaProps) {
      ...
      <div className="flex items-center gap-2.5 px-4 h-16 border-b border-slate-800 flex-shrink-0">
        ...
        {!collapsed && (
          <div className="leading-tight">
            ...
            <div className="text-[9px] text-blue-300 whitespace-nowrap">
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        <ul className="flex flex-col gap-1 px-2">
          {navItems.map((item) => {
            ...
            const active =
              pathname === item.href ||
              (item.href !== "/contratista" && pathname.startsWith(item.href));
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
